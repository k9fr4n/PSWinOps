...
Describe -Name 'Set-NTPClient' -Fixture {
    Context -Name 'Nominal - service running, w32tm outputs valid data' -Fixture {
        BeforeEach {
            ...
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith { [PSCustomObject]@{ Output = ''; ExitCode = 0 } } -ParameterFilter { ($ArgumentList -join ' ') -match '/register' }
            ...
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith { [PSCustomObject]@{ Output = ''; ExitCode = 0 } } -ParameterFilter { ($ArgumentList -join ' ') -match '/config /update' }
        ...
        It -Name 'Should invoke w32tm /query /source for verification' -Test {
            Set-NTPClient -NtpServers 'ntp1.example.com' -Confirm:$false
            Should -Invoke -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -Times 1 -Exactly -ParameterFilter { ($ArgumentList -join ' ') -match '/query.*source' }
        }
    }
    ...
    Context -Name 'WhatIf - should not perform any changes' -Fixture {
        BeforeEach {
            ...
            Mock -CommandName 'Set-ItemProperty' -ModuleName 'PSWinOps' -MockWith {}
            Mock -CommandName 'Restart-Service' -ModuleName 'PSWinOps' -MockWith {}
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith { [PSCustomObject]@{ Output = ''; ExitCode = 0 } }
        }
    ...
    Context -Name 'Locale-agnostic - German output with exit code 0' -Fixture {
        BeforeEach {
            ...
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith { [PSCustomObject]@{ Output = 'ntp1.example.com'; ExitCode = 0 } } -ParameterFilter { ($ArgumentList -join ' ') -match '/query.*source' }
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith { [PSCustomObject]@{ Output = ''; ExitCode = 0 } } -ParameterFilter { ($ArgumentList -join ' ') -match '/config /update' }
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith { [PSCustomObject]@{ Output = ($script:mockSyncOutputDE -join "`r`n"); ExitCode = 0 } } -ParameterFilter { ($ArgumentList -join ' ') -match '/resync' }
        }
    ...
    Context -Name 'Locale-agnostic - French output with exit code 0' -Fixture {
        BeforeEach {
            ...
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith {
                [PSCustomObject]@{
                    Output   = ("Envoi de la commande de resynchronisation à l'ordinateur local", "La commande s'est terminée correctement.") -join "`r`n"
                    ExitCode = 0
                }
            } -ParameterFilter { ($ArgumentList -join ' ') -match '/resync' }
        }
    ...
    Context -Name 'Service not found - w32tm register fallback' -Fixture {
        BeforeEach {
            ...
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith { [PSCustomObject]@{ Output = ''; ExitCode = 0 } } -ParameterFilter { ($ArgumentList -join ' ') -match '/register' }
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith { [PSCustomObject]@{ Output = ''; ExitCode = 0 } } -ParameterFilter { ($ArgumentList -join ' ') -match '/config /update' }
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith { [PSCustomObject]@{ Output = 'OK'; ExitCode = 0 } } -ParameterFilter { ($ArgumentList -join ' ') -match '/resync' }
            Mock -CommandName 'Get-ItemProperty' -ModuleName 'PSWinOps' -MockWith {
                [PSCustomObject]@{ NtpServer = 'ntp1.example.com,0x9' }
            } -ParameterFilter { $Name -eq 'NtpServer' }
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith { [PSCustomObject]@{ Output = 'ntp1.example.com'; ExitCode = 0 } } -ParameterFilter { ($ArgumentList -join ' ') -match '/query.*source' }
        ...
        It -Name 'Should call w32tm /register when service is not found initially' -Test {
            Set-NTPClient -NtpServers 'ntp1.example.com' -Confirm:$false
            Should -Invoke -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -Times 1 -Exactly -ParameterFilter { ($ArgumentList -join ' ') -match '/register' }
        }
    }
    ...
    Context -Name 'Verification step failures - Get-ItemProperty and w32tm /query /source fail' -Fixture {
        BeforeEach {
            ...
            } -ParameterFilter { ($ArgumentList -join ' ') -match '/query.*source' }
        }
