...
Describe -Name 'Get-NTPConfiguration' -Fixture {
    ...
    Context -Name 'Nominal - service running, w32tm outputs valid data' -Fixture {
        ...
        BeforeEach {
            ...
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith {
                [PSCustomObject]@{ Output = ($script:mockConfigOutput -join "`r`n"); ExitCode = 0 }
            } -ParameterFilter { $ArgumentList -contains '/query' -and $ArgumentList -contains '/configuration' }

            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith {
                [PSCustomObject]@{ Output = ($script:mockStatusOutput -join "`r`n"); ExitCode = 0 }
            } -ParameterFilter { $ArgumentList -contains '/query' -and $ArgumentList -contains '/status' }

            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith {
                [PSCustomObject]@{ Output = ($script:mockPeersOutput -join "`r`n"); ExitCode = 0 }
            } -ParameterFilter { $ArgumentList -contains '/query' -and $ArgumentList -contains '/peers' }
        }
    ...
    Context -Name 'Degraded - w32tm returns empty output' -Fixture {
        ...
        BeforeEach {
            ...
            # Mock without ParameterFilter = catch-all for all Invoke-NativeCommand calls
            Mock -CommandName 'Invoke-NativeCommand' -ModuleName 'PSWinOps' -MockWith {
    ...
    Context -Name 'Remote execution via Invoke-Command' -Fixture {

        BeforeEach {
            Mock -CommandName 'Invoke-Command' -ModuleName 'PSWinOps' -MockWith {
                @{
                    ServiceStatus = 'Running'
                    Config        = $script:mockConfigOutput
                    Status        = $script:mockStatusOutput
                    Peers         = $script:mockPeersOutput
                }
            }
        }

        It -Name 'Should use Invoke-Command for remote target' -Test {
            Get-NTPConfiguration -ComputerName 'REMOTE01'
            Should -Invoke -CommandName 'Invoke-Command' -ModuleName 'PSWinOps' -Times 1 -Exactly
        }

        It -Name 'Should return result with correct ComputerName for remote' -Test {
            $script:remoteResult = Get-NTPConfiguration -ComputerName 'REMOTE01'
            $script:remoteResult.ComputerName | Should -Be 'REMOTE01'
        }

        It -Name 'Should parse remote w32tm output correctly' -Test {
            $script:remoteResult = Get-NTPConfiguration -ComputerName 'REMOTE01'
            $script:remoteResult.SyncType | Should -Match 'NTP'
            $script:remoteResult.CurrentSource | Should -Be 'ntp1.example.com'
        }
    }

    Context -Name 'Remote execution with IncludePeerDetails' -Fixture {
        ...
        It -Name 'Should include PeerDetails for remote with -IncludePeerDetails' -Test {
            $script:remotePeerResult = Get-NTPConfiguration -ComputerName 'REMOTE01' -IncludePeerDetails
            $script:remotePeerResult.PSObject.Properties.Name | Should -Contain 'PeerDetails'
        }
    }
