...
Describe 'Get-NetworkConnection' {
    ...
    Context 'Parameter validation' {
        It 'Should have CmdletBinding' {
            $cmd = Get-Command -Name 'Get-NetworkConnection'
            $cmd.CmdletBinding | Should -BeTrue
        }

        It 'Should have OutputType PSWinOps.NetworkConnection' {
            $cmd = Get-Command -Name 'Get-NetworkConnection'
            $cmd.OutputType.Name | Should -Contain 'PSWinOps.NetworkConnection'
        }
        ...
        It 'Should reject invalid Protocol value' {
            { Get-NetworkConnection -Protocol 'ICMP' } | Should -Throw
        }

        It 'Should reject invalid State value' {
            { Get-NetworkConnection -State 'InvalidState' } | Should -Throw
        }

        It 'Should reject LocalPort out of range' {
            { Get-NetworkConnection -LocalPort 0 } | Should -Throw
            { Get-NetworkConnection -LocalPort 70000 } | Should -Throw
        }

        It 'Should reject empty ComputerName' {
            { Get-NetworkConnection -ComputerName '' } | Should -Throw
        }

        It 'Should reject null ComputerName' {
            { Get-NetworkConnection -ComputerName $null } | Should -Throw
        }
    }
    ...
    Context 'Happy path - local machine with TCP only' {
        ...
        It 'Should return objects with PSTypeName PSWinOps.NetworkConnection' {
            $results = Get-NetworkConnection -Protocol TCP
            $results | ForEach-Object {
                $_.PSObject.TypeNames[0] | Should -Be 'PSWinOps.NetworkConnection'
            }
        }
    ...
    Context 'Happy path - local machine with UDP only' {
        ...
        It 'Should return UDP entries with State Stateless' {
            $results = Get-NetworkConnection -Protocol UDP
    ...
    Context 'Happy path - explicit remote machine' {
        ...
        It 'Should use Invoke-Command for remote machine' {
            $results = Get-NetworkConnection -ComputerName 'REMOTESRV01' -Protocol TCP
    ...
    Context 'Pipeline - multiple machine names' {
        ...
        It 'Should accept multiple computers via pipeline' {
            $results = 'REMOTE01', 'REMOTE02' | Get-NetworkConnection -Protocol TCP
    ...
    Context 'Per-machine failure - continues and writes error' {
        ...
        It 'Should write error for failed machine but not throw' {
            $results = Get-NetworkConnection -ComputerName 'BADSRV01' -Protocol TCP -ErrorVariable err -ErrorAction SilentlyContinue
        ...
        It 'Should continue processing remaining machines after failure' {
            ...
            $results = Get-NetworkConnection -ComputerName 'BADSRV', 'GOODSRV' -Protocol TCP -ErrorAction SilentlyContinue
    ...
    Context 'Filtering - State parameter' {
        ...
        It 'Should pass State to Get-NetTCPConnection' {
            ...
            $results = Get-NetworkConnection -Protocol TCP -State Established
            $results | Should -HaveCount 1
    ...
    Context 'Filtering - ProcessName parameter' {
        ...
        It 'Should filter by exact process name' {
            $results = Get-NetworkConnection -Protocol TCP -ProcessName 'nginx'
            $results | Should -HaveCount 1
            $results[0].ProcessName | Should -Be 'nginx'
        }

        It 'Should filter by wildcard process name' {
            $results = Get-NetworkConnection -Protocol TCP -ProcessName 'svc*'
            $results | Should -HaveCount 1
    ...
    Context 'Filtering - LocalPort parameter' {
        ...
        It 'Should filter by local port' {
            $results = Get-NetworkConnection -Protocol TCP -LocalPort 443
            $results | Should -HaveCount 1
    ...
    Context 'Output object shape' {
        ...
        It 'Should have all expected properties' {
            $result = Get-NetworkConnection -Protocol TCP | Select-Object -First 1
