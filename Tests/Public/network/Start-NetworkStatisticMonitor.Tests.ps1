...
Describe 'Start-NetworkStatisticMonitor' {

    Context 'Parameter validation' {
        It 'Should have CmdletBinding' {
            $cmd = Get-Command -Name 'Start-NetworkStatisticMonitor'
        ...
        It 'Should reject invalid Protocol value' {
            { Start-NetworkStatisticMonitor -Protocol 'ICMP' } | Should -Throw
        }

        It 'Should reject RefreshInterval of 0' {
            { Start-NetworkStatisticMonitor -RefreshInterval 0 } | Should -Throw
        }

        It 'Should reject RefreshInterval above 300' {
            { Start-NetworkStatisticMonitor -RefreshInterval 301 } | Should -Throw
        }
        ...
        It 'Should reject empty ComputerName' {
            { Start-NetworkStatisticMonitor -ComputerName '' } | Should -Throw
        }

        It 'Should reject null ComputerName' {
            { Start-NetworkStatisticMonitor -ComputerName $null } | Should -Throw
        }
    }
    ...
    Context 'Monitor loop - calls Get-NetworkConnection and displays results' {
        ...
        It 'Should not return pipeline objects' {
            $results = Start-NetworkStatisticMonitor -Protocol TCP -ErrorAction SilentlyContinue 2>$null
        ...
        It 'Should call Start-Sleep with the RefreshInterval' {
            Start-NetworkStatisticMonitor -RefreshInterval 5 -Protocol TCP -ErrorAction SilentlyContinue 2>$null
